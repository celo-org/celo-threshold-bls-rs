...
mod dkg_mod {
    ...
    impl<'a, P: JsonRpcClient, S: Signer> DKG<P, S> {
        ...
        #[doc = "Calls the contract's `submitPublicPolynomial` (0x88ddf132) function"]
        pub fn submit_public_polynomial(
            &self,
            public_polynomial: Vec<u8>,
        ) -> ContractCall<P, S, H256> {
            self.0
                .method_hash([136, 221, 241, 50], (public_polynomial,))
                .expect("method not found (this should never happen)")
        }
        #[doc = "Calls the contract's `publicPolynomial` (0x55629dd6) function"]
        pub fn public_polynomial(&self) -> ContractCall<P, S, Vec<u8>> {
            self.0
                .method_hash([85, 98, 157, 214], ())
                .expect("method not found (this should never happen)")
        }
